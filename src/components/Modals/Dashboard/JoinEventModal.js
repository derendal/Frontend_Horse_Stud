...
export default function JoinEventModal({user,event,openModal,setOpenModal,cancelButtonRef}) {
    const [input, setInput] = useState({})
    const [errors, setErrors] = useState({})
    const [stud,setStud] = useState({})
    const [horses,setHorses] = useState([{}])
    const sendHorse = (item) => {
        console.log("uuuuser",user)
        console.log("event",event)
        post_request(
            'http://127.0.0.1:8080/event/new/eventlist',
            {
                event: event,
                horse: item,
                gamer: user.user.id
        ...
        if(user !== undefined && user.user !== null){
            get_request(
                ...
                if(response.body.length > 0){
                    ...
                    let userStud = studs.find(s => s.gamerId.gamerId === user.user.id);
